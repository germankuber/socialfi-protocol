		...
		<div className="flex items-center gap-3 flex-1 min-w-0">
			...
			<div className="min-w-0">
				<div className="flex items-center gap-1.5">
					<Link
						to={`/profile/${address}`}
						className="text-sm font-medium hover:text-brand-500 transition-colors flex items-center gap-1"
					>
						{profile?.name || truncated}
						{profile?.verified && <VerifiedBadge size="sm" />}
					</Link>
					{profile?.verified ? (
						<span className="inline-flex items-center rounded-full bg-success/10 px-1.5 py-0.5 text-[9px] font-semibold text-success shrink-0">
							Verified
						</span>
					) : (
			...
			<div className="space-y-4">
				<div className="panel space-y-3">
					...
					<div className="flex gap-2">
						...
						<button
							onClick={followUser}
							disabled={!target.trim() || !account || busy}
