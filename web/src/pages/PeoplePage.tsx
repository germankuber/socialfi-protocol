		...
		<div className="space-y-6 animate-fade-in">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-secondary text-sm mt-1">All registered users on the protocol.</p>
				</div>
			...
			{loading && users.length === 0 ? (
				...
				<div className="panel text-center py-12 text-secondary">No users registered yet.</div>
			) : (
					...
					{users
						...
						>
							<div className="flex items-center gap-4">
								{/* Avatar */}
								{user.avatar ? (
									<img src={user.avatar} alt={user.name} className="w-14 h-14 rounded-full object-cover bg-surface-800 shrink-0" />
								) : (
									<div className="w-14 h-14 rounded-full bg-brand-500/20 flex items-center justify-center text-brand-500 text-lg font-bold shrink-0">
										{user.name?.[0]?.toUpperCase() || user.address.slice(2, 4)}
									</div>
								)}
								...
								<div className="flex-1 min-w-0">
									...
									{user.bio && (
										<p className="text-xs text-secondary mt-0.5 line-clamp-1">{user.bio}</p>
									)}
									<div className="flex items-center gap-3 mt-1 text-[11px] text-surface-500">
										...
										<span className="font-mono">Block #{user.createdAt}</span>
									</div>
								</div>
