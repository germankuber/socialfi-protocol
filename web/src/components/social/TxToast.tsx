...
const STAGE_CONFIG: Record<
	TxStage,
	{ color: string; bg: string; border: string; icon: "spinner" | "check" | "error" | "none" }
> = {
...
export default function TxToast({ state, onDismiss }: TxToastProps) {
	...
	return (
		<div className="fixed bottom-6 right-6 z-50 animate-slide-up w-96 max-w-[calc(100vw-2rem)]">
			<div
				className={`rounded-2xl border ${config.border} ${config.bg} backdrop-blur-xl p-4 shadow-lg`}
			>
				{/* Progress bar */}
				...
				<div className="flex items-start gap-3">
					...
					<div className="flex-1 min-w-0">
						...
						<p className="text-xs text-secondary mt-0.5 break-words">{state.message}</p>
						{state.blockHash && (
							<a
